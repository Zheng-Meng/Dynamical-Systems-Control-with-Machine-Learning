...
Now it comes to the data generation part. The control signals are chosen within a specific range, generated by Gaussian noise and then smoothed. The control signals, as well as the time series affected by them, are shown below:
...
On the left are the control signals generated by reservoir computing, where they are limited within the bounds set in training (even if the RC predicts values out of range). The controlled time series, compared with the ground-truth target, are shown on the right. It can be seen that the RC tracks the target periodic orbit well visually.
...
- (Future space) Many other interesting ideas came to my mind, but I’m tired to write them in detail right now.
